...
void* ape_ast_destroy_expr(ApeExpression_t* expr)
{
    ...
    switch(expr->type)
    {
        ...
        {
            ptrarray_destroywithitems(expr->array, (ApeDataCallback_t)ape_ast_destroy_expr);
            break;
        }
        case EXPRESSION_MAP_LITERAL:
        {
            ptrarray_destroywithitems(expr->map.keys, (ApeDataCallback_t)ape_ast_destroy_expr);
            ptrarray_destroywithitems(expr->map.values, (ApeDataCallback_t)ape_ast_destroy_expr);
            break;
        }
        ...
        case EXPRESSION_FUNCTION_LITERAL:
        {
            ...
            ptrarray_destroywithitems(fn->params, (ApeDataCallback_t)ape_ast_destroy_ident);
            ape_ast_destroy_codeblock(fn->body);
            break;
        }
        case EXPRESSION_CALL:
        {
            ptrarray_destroywithitems(expr->callexpr.args, (ApeDataCallback_t)ape_ast_destroy_expr);
...
ApeExpression_t* ape_ast_copy_expr(ApeExpression_t* expr)
{
    ...
    switch(expr->type)
    {
        ...
        case EXPRESSION_ARRAY_LITERAL:
        {
            ApePtrArray_t* valuescopy = ptrarray_copywithitems(expr->array, (ApeDataCallback_t)ape_ast_copy_expr, (ApeDataCallback_t)ape_ast_destroy_expr);
            if(!valuescopy)
            {
                return NULL;
            }
            ...
            if(!res)
            {
                ptrarray_destroywithitems(valuescopy, (ApeDataCallback_t)ape_ast_destroy_expr);
                return NULL;
            }
            break;
        }
        case EXPRESSION_MAP_LITERAL:
        {
            ApePtrArray_t* keyscopy = ptrarray_copywithitems(expr->map.keys, (ApeDataCallback_t)ape_ast_copy_expr, (ApeDataCallback_t)ape_ast_destroy_expr);
            ApePtrArray_t* valuescopy = ptrarray_copywithitems(expr->map.values, (ApeDataCallback_t)ape_ast_copy_expr, (ApeDataCallback_t)ape_ast_destroy_expr);
            if(!keyscopy || !valuescopy)
            {
                ptrarray_destroywithitems(keyscopy, (ApeDataCallback_t)ape_ast_destroy_expr);
                ptrarray_destroywithitems(valuescopy, (ApeDataCallback_t)ape_ast_destroy_expr);
                return NULL;
            }
        ...
        case EXPRESSION_FUNCTION_LITERAL:
        {
            ApePtrArray_t* paramscopy = ptrarray_copywithitems(expr->fnliteral.params, (ApeDataCallback_t)ape_ast_copy_ident,(ApeDataCallback_t) ape_ast_destroy_ident);
            ...
            {
                ptrarray_destroywithitems(paramscopy, (ApeDataCallback_t)ape_ast_destroy_ident);
        ...
        case EXPRESSION_CALL:
        {
            ...
            ApePtrArray_t* argscopy = ptrarray_copywithitems(expr->callexpr.args, (ApeDataCallback_t)ape_ast_copy_expr, (ApeDataCallback_t)ape_ast_destroy_expr);
...
void* ape_ast_destroy_stmt(ApeStatement_t* stmt)
{
    ...
    switch(stmt->type)
    {
        ...
        case STATEMENT_IF:
            {
                ptrarray_destroywithitems(stmt->ifstatement.cases, (ApeDataCallback_t)ape_ast_destroy_ifcase);
...
ApeStatement_t* ape_ast_copy_stmt(const ApeStatement_t* stmt)
{
    ...
    switch(stmt->type)
    {
        ...
        case STATEMENT_IF:
            {
                ApePtrArray_t* casescopy = ptrarray_copywithitems(stmt->ifstatement.cases, (ApeDataCallback_t)ape_ast_copy_ifcase, (ApeDataCallback_t)ape_ast_destroy_ifcase);
                ApeCodeblock_t* alternativecopy = ape_ast_copy_codeblock(stmt->ifstatement.alternative);
                if(!casescopy || !alternativecopy)
                {
                    ptrarray_destroywithitems(casescopy, (ApeDataCallback_t)ape_ast_destroy_ifcase);
...
void* ape_ast_destroy_codeblock(ApeCodeblock_t* block)
{
    ...
    ptrarray_destroywithitems(block->statements, (ApeDataCallback_t)ape_ast_destroy_stmt);
...
ApeCodeblock_t* ape_ast_copy_codeblock(ApeCodeblock_t* block)
{
    ...
    statementscopy = ptrarray_copywithitems(block->statements, (ApeDataCallback_t)ape_ast_copy_stmt, (ApeDataCallback_t)ape_ast_destroy_stmt);
    if(!statementscopy)
    {
        return NULL;
    }
    ...
    if(!res)
    {
        ptrarray_destroywithitems(statementscopy, (ApeDataCallback_t)ape_ast_destroy_stmt);
        return NULL;
    }
    return res;
}
...
ApeStatement_t* ape_parser_parseifstmt(ApeParser_t* p)
{
    ...
    return res;
err:
    ptrarray_destroywithitems(cases, (ApeDataCallback_t)ape_ast_destroy_ifcase);
...
ApeExpression_t* ape_parser_parseexpr(ApeParser_t* p, ApePrecedence_t prec)
{
    ...
    leftexpr->pos = pos;
    while(!lexer_cur_token_is(&p->lexer, TOKEN_SEMICOLON) && prec < ape_parser_getprecedence(p->lexer.curtoken.type))
    {
...
ApeExpression_t* ape_parser_parseliteralarray(ApeParser_t* p)
{
    ...
    if(!res)
    {
        ptrarray_destroywithitems(array, (ApeDataCallback_t)ape_ast_destroy_expr);
        return NULL;
    }
    return res;
}
...
ApeExpression_t* ape_parser_parseliteralmap(ApeParser_t* p)
{
    ...
    return res;
err:
    ptrarray_destroywithitems(keys, (ApeDataCallback_t)ape_ast_destroy_expr);
    ptrarray_destroywithitems(values, (ApeDataCallback_t)ape_ast_destroy_expr);
    return NULL;
}
...
err:
    ape_ast_destroy_codeblock(body);
    ptrarray_destroywithitems(params, (ApeDataCallback_t)ape_ast_destroy_ident);
    p->depth -= 1;
    return NULL;
}
...
ApeExpression_t* ape_parser_parsecallexpr(ApeParser_t* p, ApeExpression_t* left)
{
    ...
    if(!res)
    {
        ptrarray_destroywithitems(args, (ApeDataCallback_t)ape_ast_destroy_expr);
        return NULL;
    }
    return res;
}
...
ApePtrArray_t* ape_parser_parseexprlist(ApeParser_t* p, ApeTokenType_t starttoken, ApeTokenType_t endtoken, bool trailingcommaallowed)
{
    ...
    return res;
err:
    ptrarray_destroywithitems(res, (ApeDataCallback_t)ape_ast_destroy_expr);
    return NULL;
}
