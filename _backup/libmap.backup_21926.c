...
bool valdict_setvalueat(const ApeValDictionary_t* dict, ApeSize_t ix, const void* value)
{
    ApeSize_t offset;
    if(ix >= dict->count)
    {
...
ApeSize_t valdict_getcellindex(const ApeValDictionary_t* dict, const void* key, unsigned long hash, bool* out_found)
{
    bool are_equal;
    ApeSize_t ofs;
    ApeSize_t i;
...
bool valdict_setkeyat(ApeValDictionary_t* dict, ApeSize_t ix, void* key)
{
    ApeSize_t offset;
    if(ix >= dict->count)
    {
...
ApeSize_t strdict_getcellindex(const ApeStrDictionary_t* dict, const char* key, unsigned long hash, bool* out_found)
{
    ApeSize_t i;
    ApeSize_t ix;
    ApeSize_t cell;
...
ApeObject_t object_make_map(ApeContext_t* ctx)
{
    return object_make_mapcapacity(ctx, 32);
}

ApeObject_t object_make_mapcapacity(ApeContext_t* ctx, unsigned capacity)
{
    ...
    data->map = valdict_makecapacity(ctx, capacity, sizeof(ApeObject_t), sizeof(ApeObject_t));
    if(!data->map)
    {
        return object_make_null(ctx);
    }
...
bool object_map_setnamedvalue(ApeObject_t obj, const char* key, ApeObject_t value)
{
    ...
    ApeObject_t key_object = object_make_string(mem->context, key);
...
bool object_map_setnamedstring(ApeObject_t obj, const char* key, const char* string)
{
    ...
    ApeObject_t string_object = object_make_string(mem->context, string);
