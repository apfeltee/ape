...
void valdict_destroy_with_items(ApeValDictionary_t* dict)
{
    ApeSize_t i;
    void** vp;
    if(!dict)
...
void valdict_set_copyfunc(ApeValDictionary_t* dict, ApeDataCallback_t fn)
{
    dict->copy_fn = fn;
}

void valdict_set_deletefunc(ApeValDictionary_t* dict, ApeDataCallback_t fn)
{
    dict->destroy_fn = fn;
}

ApeValDictionary_t* valdict_copy_with_items(ApeValDictionary_t* dict)
{
    ApeSize_t i;
    bool ok;
    const char* key;
    ...
    valdict_set_copyfunc(dict_copy, (ApeDataCallback_t)dict->copy_fn);
    valdict_set_deletefunc(dict_copy, (ApeDataCallback_t)dict->destroy_fn);
    ...
    {
        key = valdict_get_key_at(dict, i);
        item = valdict_get_value_at(dict, i);
        item_copy = dict_copy->copy_fn(item);
        if(item && !item_copy)
        {
            valdict_destroy_with_items(dict_copy);
            return NULL;
        }
...
void strdict_destroy_with_items(ApeStrDictionary_t* dict)
{
    ApeSize_t i;
    if(!dict)
    {
...
ApeStrDictionary_t* strdict_copy_with_items(ApeStrDictionary_t* dict)
{
    ApeSize_t i;
    bool ok;
    const char* key;
    ...
    {
        key = strdict_get_key_at(dict, i);
        item = strdict_get_value_at(dict, i);
        item_copy = dict_copy->copy_fn(item);
        if(item && !item_copy)
        {
            strdict_destroy_with_items(dict_copy);
            return NULL;
        }
...
{
    return strdict_set_internal(dict, key, NULL, value);
}
...
void* strdict_get(const ApeStrDictionary_t* dict, const char* key)
{
    ...
    found = false;
    cell_ix = strdict_get_cell_ix(dict, key, hash, &found);
    if(found == false)
    {
        return NULL;
    }
...
void* strdict_get_value_at(const ApeStrDictionary_t* dict, unsigned int ix)
{
    if(ix >= dict->count)
    {
        return NULL;
...
const char* strdict_get_key_at(const ApeStrDictionary_t* dict, unsigned int ix)
{
    if(ix >= dict->count)
    {
        return NULL;
...
unsigned int strdict_get_cell_ix(const ApeStrDictionary_t* dict, const char* key, unsigned long hash, bool* out_found)
{
    ApeSize_t i;
    ApeSize_t ix;
    ApeSize_t cell;
...
bool strdict_grow_and_rehash(ApeStrDictionary_t* dict)
{
    bool ok;
    ApeSize_t i;
    char* key;
    ...
    for(i = 0; i < dict->count; i++)
    {
        ...
        key = dict->keys[i];
        value = dict->values[i];
        ok = strdict_set_internal(&new_dict, key, key, value);
        if(!ok)
        {
...
bool strdict_set_internal(ApeStrDictionary_t* dict, const char* ckey, char* mkey, void* value)
{
    bool ok;
    bool found;
    ApeSize_t cklen;
    ...
    found = false;
    cell_ix = strdict_get_cell_ix(dict, ckey, hash, &found);
    if(found)
    {
        item_ix = dict->cells[cell_ix];
    ...
    if(dict->count >= dict->item_capacity)
    {
        ok = strdict_grow_and_rehash(dict);
        if(!ok)
        {
            return false;
        }
...
ApeObject_t object_make_map_with_capacity(ApeGCMemory_t* mem, unsigned capacity)
{
    ...
    valdict_set_hash_function(data->map, (ApeDataHashFunc_t)object_value_hash);
    valdict_set_equals_function(data->map, (ApeDataEqualsFunc_t)object_value_wrapequals);
    return object_make_from_data(APE_OBJECT_MAP, data);
}
