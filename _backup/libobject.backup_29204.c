...
bool object_isnull(ApeObject_t obj)
{
...
bool object_isallocated(ApeObject_t object)
{
    return (object.handle & OBJECT_ALLOCATED_HEADER) == OBJECT_ALLOCATED_HEADER;
}

bool object_ishashable(ApeObject_t obj)
{
    ApeObjectType_t type;
    type = object_get_type(obj);
    switch(type)
...
uint64_t object_gettypetag(ApeObjectType_t type)
{
    switch(type)
    {
        case APE_OBJECT_NONE:
...
ApeObject_t object_make_from_data(ApeObjectType_t type, ApeObjectData_t* data)
{
    ...
    type_tag = object_gettypetag(type) & 0x7;
        ...
        case APE_OBJECT_FUNCTION:
            {
                ...
                for(i = 0; i < function->free_vals_count; i++)
                {
                    ...
                    if(!object_isnull(free_val) && object_isnull(free_val_copy))
                    {
                        return object_make_null();
                    }
        ...
        case APE_OBJECT_ARRAY:
            {
                ...
                for(i = 0; i < len; i++)
                {
                    ...
                    if(!object_isnull(item) && object_isnull(item_copy))
                    {
                        return object_make_null();
                    }
        ...
        case APE_OBJECT_MAP:
            {
                ...
                for(i = 0; i < object_map_getlength(obj); i++)
                {
                    ...
                    if(!object_isnull(key) && object_isnull(key_copy))
                    {
                        return object_make_null();
                    }
                    val_copy = object_deep_copy_internal(mem, val, copies);
                    if(!object_isnull(val) && object_isnull(val_copy))
                    {
                        return object_make_null();
                    }
...
ApeFloat_t object_compare(ApeObject_t a, ApeObject_t b, bool* out_ok)
{
    ...
    *out_ok = true;
    ...
    else if((object_isallocated(a) || object_isnull(a)) && (object_isallocated(b) || object_isnull(b)))
    {
...
{
    APE_ASSERT(object_isallocated(object) || object_get_type(object) == APE_OBJECT_NULL);
