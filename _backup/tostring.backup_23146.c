...
static bool ape_tostrign_opcodecoderead(ApeOpcodeDefinition_t* def, ApeUShort_t* instr, uint64_t out_operands[2])
{
    ApeSize_t i;
    int offset = 0;
...
bool ape_tostring_statement(const ApeStatement_t* stmt, ApeWriter_t* buf)
{
    ...
    switch(stmt->type)
    {
        case STATEMENT_DEFINE:
        {
            ...
            if(def_stmt->value)
            {
                ape_tostring_expression(def_stmt->value, buf);
            }
        ...
        case STATEMENT_IF:
        {
            ...
            ape_tostring_expression(if_case->test, buf);
            ape_writer_append(buf, ") ");
            ape_tostring_codeblock(if_case->consequence, buf);
            for(i = 1; i < ape_ptrarray_count(stmt->ifstatement.cases); i++)
            {
                ...
                ape_tostring_expression(elif_case->test, buf);
                ape_writer_append(buf, ") ");
                ape_tostring_codeblock(elif_case->consequence, buf);
            }
        ...
        case EXPRESSION_ARRAY_LITERAL:
            {
                ...
                for(i = 0; i < ape_ptrarray_count(expr->array); i++)
                {
                    ...
                    ape_tostring_expression(arr_expr, buf);
        ...
        case EXPRESSION_MAP_LITERAL:
            {
                ...
                for(i = 0; i < ape_ptrarray_count(map->keys); i++)
                {
                    ...
                    ape_tostring_expression(key_expr, buf);
                    ape_writer_append(buf, " : ");
                    ape_tostring_expression(val_expr, buf);
