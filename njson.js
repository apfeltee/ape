...
{
    while(state.m_index < state.m_source.length && JSON["isWhiteSpace"](state.m_source[state.m_index]))
    {
        state.m_index++;
    }
};
...
JSON["parseValue"] = function(state)
{
    ...
    Fiber.abort("Invalid json (c=" + c + ", index=" + state.m_index + ", max=" + state.m_source.length + ")");
};
